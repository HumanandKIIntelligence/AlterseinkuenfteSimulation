...
def _restschuld_vergleich_ui(restschuld: float, endjahr: int, d: dict, _rc: int) -> None:
    ...
    # ── Optionsvergleich: Startjahr + Laufzeit des Anschlusskredits ───────────
    if behandlung not in ("ratenkredit", "einmalzahlungen"):
        return

    if behandlung == "einmalzahlungen":
        _ezl_ov = list(d.get("anschluss_einmalzahlungen", []))
        _eff_rs = max(0.0, restschuld - sum(
            float(e["betrag"]) for e in _ezl_ov if int(e["jahr"]) <= endjahr
        ))
    else:
        _eff_rs = restschuld
    if _eff_rs <= 0:
    ...
    _ak_rate_ov   = _annuitaet_rate(_eff_rs, anschluss_zins, anschluss_laufzeit)
    _ak_zinsen_ov = _ak_rate_ov * anschluss_laufzeit - _eff_rs
    _ak_start_yr  = endjahr + 1
    _ak_end_yr    = endjahr + anschluss_laufzeit
    _ak_years     = list(range(_ak_start_yr, _ak_end_yr + 1))

    _jd_raw = st.session_state.get("_sb_eo_jd", [])
    _jd: dict = {int(row["Jahr"]): row for row in _jd_raw} if _jd_raw else {}
    _has_jd = bool(_jd)

    # Kapital und Netto zum Startjahr
    _kap_start  = float(_jd.get(endjahr, {}).get("Kap_Pool", 0)) if _has_jd else 0.0
    _netto_sj   = float(_jd.get(endjahr, {}).get("Netto",    0)) if _has_jd else 0.0

    # Kapitalerträge über AK-Laufzeit:
    # Rendite_Jahr = Kap_Pool[y] - Kap_Pool[y-1] + Src_Kapitalverzehr[y] - Kap_Injektion[y]
    _kap_ertrag   = 0.0
    _prev_kap     = _kap_start
    _netto_ak_yrs: list[float] = []
    _inj_total    = 0.0
    for _y in _ak_years:
        if _y in _jd:
            _yr = _jd[_y]
            _kap_y   = float(_yr.get("Kap_Pool",          0))
            _inj_y   = float(_yr.get("Kap_Injektion",     0))
            _vzr_y   = float(_yr.get("Src_Kapitalverzehr",0)) + float(_yr.get("Kap_Sonder_Tilgung", 0))
            _kap_ertrag += _kap_y - _prev_kap + _vzr_y - _inj_y
            _prev_kap    = _kap_y
            _inj_total  += _inj_y
            _netto_ak_yrs.append(float(_yr.get("Netto", 0)))

    _netto_min = min(_netto_ak_yrs) if _netto_ak_yrs else 0.0
    _netto_max = max(_netto_ak_yrs) if _netto_ak_yrs else 0.0
    _partial   = len(_netto_ak_yrs) < anschluss_laufzeit and _has_jd

    st.divider()
    st.caption(f"**📊 Optionsvergleich** {'*(Teildaten – Horizont endet vor AK-Laufzeit)*' if _partial else ''}")

    _ov1, _ov2 = st.columns(2)
    with _ov1:
        st.caption(f"Zum Startjahr **{_ak_start_yr}**")
        _ovc1, _ovc2 = st.columns(2)
        _ovc1.metric("Kapital", f"{_de(_kap_start)} €" if _has_jd else "–",
                     help="Kapitalanlage-Pool am Ende des Hypothek-Jahres (aus Entnahme-Optimierung).")
        _ovc2.metric("Nettogehalt", f"{_de(_netto_sj)} €/J." if _has_jd else "–",
                     help="Jährliches Nettoeinkommen im Startjahr des Anschlusskredits.")
    with _ov2:
        st.caption(f"Über Laufzeit **{anschluss_laufzeit} Jahre** ({_ak_start_yr}–{_ak_end_yr})")
        _ovc3, _ovc4 = st.columns(2)
        _inj_hint = f" (inkl. {_de(_inj_total)} € Vorsorge-Einzahlungen)" if _inj_total > 0 else ""
        _ovc3.metric("Kapitalerträge", f"{_de(_kap_ertrag)} €" if _has_jd else "–",
                     help=f"Nettorendite des Kapitalanlage-Pools über die AK-Laufzeit{_inj_hint}.")
        _ovc4.metric("AK-Zinslast", f"{_de(_ak_zinsen_ov)} €",
                     help=f"Gesamtzinsen Anschlusskredit bei {anschluss_zins*100:.2f} %, {anschluss_laufzeit} J.")

    _ovn1, _ovn2 = st.columns(2)
    _ovn1.metric("Netto min. (AK-Zeitraum)", f"{_de(_netto_min)} €/J." if _netto_ak_yrs else "–",
                 help="Niedrigstes Jahresnetto im AK-Zeitraum (aus Entnahme-Optimierung).")
    _ovn2.metric("Netto max. (AK-Zeitraum)", f"{_de(_netto_max)} €/J." if _netto_ak_yrs else "–",
                 help="Höchstes Jahresnetto im AK-Zeitraum (aus Entnahme-Optimierung).")

    if not _has_jd:
        st.caption("ℹ️ Für Kapital- und Nettowerte zuerst die **Entnahme-Optimierung** ausführen.")
